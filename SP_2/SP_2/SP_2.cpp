...
void print_menu(NoteHeader note_header) {
	wcout << "\n\n* ЗАМЕТКИ *\n\n"
		<< "файлы: " << note_header.not_empty_notes_number 
		<< "\nРазмер файла: " <<  note_header.file_size << "\n\n"
		<< "[0] - показать все заметки\n"
		<< "[1] - создать заметку\n"
		<< "[2] - изменить заметку по номеру\n"
	...
	for (int i = 0; i < notes_number; i++) {
		...
		if (note.update_count != -1) {
			...
			wcout << "\n [" << note.number << "] - |" << st.wDay << "." << st.wMonth << "." << st.wYear
				<< "|номер обновления - " << note.update_count << "|\n\t";
			wprintf(note.text);
		}
	}
...
void create_note(HANDLE file_handle, int notes_number, int note_position) {
	if (note_position == -1) {
		wcout << "\nты уже" << notes_number << " заметки. Пожалуйста, удалите некоторые, чтобы создать новый.\n";
		return;
	}
...
void modify_note(HANDLE file_handle, int notes_number, int note_position) {
	...
	if (note.update_count == -1) {
		wcout << "\nНет такой заметки с таким номером\n\n";
		return;
	}
