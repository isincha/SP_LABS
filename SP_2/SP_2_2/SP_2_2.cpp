...
void change_directory(vector<wstring> parameters, WCHAR *path) {
	if (parameters.size() != 1) {
		wprintf(L"Неверные параметры. Структура: cd <path>");
		return;
	}
	...
	switch (is_directory(new_path)) {
		case 0: wprintf(L"не может получить доступ %s\n", new_path.c_str());
			break;
		case 1: 
			wcscpy(path, new_path.c_str());
			break;
		case 2:
			wprintf(L"%s это не каталог\n", new_path.c_str());
			break;
	}		
}
...
void copy_file(vector<wstring> parameters, wstring path) {
	if (parameters.size() != 2) {
		wprintf(L"Неверные параметры. Структура: cp <file_name> <copied_file_name>\ncp <file_name> <copied_file_full_path>\ncp <file_name> <copied_folder>\n");
		return;
	}
	...
	{
		case 0: 
			wprintf(L"не может получить доступ %s\n", source_file_path.c_str());
			is_source_file_exists = false;
			break;
		case 1:
			wprintf(L"%s это каталог\n", source_file_path.c_str());
			is_source_file_exists = false;
			break;
		case 2:
	...
	if (!is_source_file_exists) {
		wprintf(L"\nНеверный исходный путь. Попробуйте еще раз.\n");
		return;
	}
	...
	if (!is_source_file_exists) {
		wprintf(L"\nНеправильный путь назначения. Попробуйте еще раз.\n");
		return;
	}
...
void create_directory(vector<wstring> parameters, wstring path) {
	if (parameters.size() != 1) {
		wprintf(L"Неверные параметры. Структура: mkdir <directory_path>\n");
		return;
	}
	...
	if (!CreateDirectory(directory_full_name, NULL)) {
		if (is_directory(directory_full_name) == 1)
			cout << "Каталог уже существует\n";
		else
			cout << "Ошибка создания каталога\n";
	}
}

void remove_directory(vector<wstring> parameters, wstring path) {
	if (parameters.size() != 1) {
		wprintf(L"Неверные параметры. Структура: rmdir <directory_path>\n");
		return;
	}
	...
	if (!RemoveDirectoryA(directory_full_name.c_str())) {
		cout << "Удаление ошибки каталога\n";
	}
}

void display_file_info(vector<wstring> parameters, wstring path) {
	if (parameters.size() != 1) {
		wprintf(L"Неверные параметры. Структура: finfo <file_name>\nfinfo <file_path>\n");
		return;
	}
	...
	if (is_directory(full_path) == 0) {
		wprintf(L"Не могу получить доступ к файлу\n");
	} else {
		WIN32_FIND_DATA fd;
		HANDLE handle = FindFirstFile(full_path.c_str(), &fd);
		if (handle != INVALID_HANDLE_VALUE)
		{
			wprintf(L"полный путь - %s\n", full_path.c_str());
			...
			printf("время последней записи - %s\n", filetime_to_string(fd.ftLastWriteTime).c_str());
			wprintf(L"полное имя - %s\n", fd.cFileName);
